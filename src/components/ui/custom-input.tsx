...
export interface CustomInputProps extends Omit<HTMLMotionProps<"input">, 'size'> {
  /**
   * Label for the input
   */
  label?: string;
...
const CustomInput = forwardRef<HTMLInputElement, CustomInputProps>(({
  ...
  const baseInputClasses = cn(
    "w-full py-2 px-3 transition-all duration-200 focus:outline-none",
    leftIcon ? 'pl-9' : '',
  ...
  const errorClasses = cn(messageClasses, "text-red-500");
  const helperClasses = cn(messageClasses, "text-gray-500");
